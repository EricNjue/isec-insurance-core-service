...
public class MotorQuoteController {
    ...
    public Mono<ResponseEntity<MotorQuoteResponse>> getMpesaPaymentStatus(@PathVariable String quoteId) {
        log.info("REST request to check M-Pesa payment status for quote: {}", quoteId);
        return orchestrator.checkPaymentStatus(quoteId)
                .map(ResponseEntity::ok);
    }
