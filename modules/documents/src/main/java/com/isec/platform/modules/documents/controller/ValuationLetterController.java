...
public class ValuationLetterController {
    ...
    public Mono<ResponseEntity<Map<String, Object>>> generate(@PathVariable Long policyId,
                                                        @RequestParam(defaultValue = "false") boolean force,
                                                        @RequestBody GenerateRequest req) {
        return valuationLetterService.generateIfNotExists(policyId,
    ...
    public Mono<ResponseEntity<ValuationLetter>> get(@PathVariable Long id) {
    ...
    @PreAuthorize("hasAnyRole('RETAIL_USER','AGENT','ADMIN')")
    public Mono<ResponseEntity<Map<String, String>>> download(@PathVariable Long id) {
        return valuationLetterRepository.findById(id)
                .switchIfEmpty(Mono.error(new IllegalArgumentException("Valuation letter not found: " + id)))
                .map(letter -> {
    ...
    @PreAuthorize("hasAnyRole('RETAIL_USER','AGENT','ADMIN')")
    public Mono<ResponseEntity<ValuationLetter>> getLatestByPolicy(@PathVariable Long policyId) {
    ...
    @PreAuthorize("hasAnyRole('RETAIL_USER','AGENT','ADMIN')")
    public Mono<ResponseEntity<ValuationLetter>> getLatestByPolicyNumber(@PathVariable String policyNumber) {
