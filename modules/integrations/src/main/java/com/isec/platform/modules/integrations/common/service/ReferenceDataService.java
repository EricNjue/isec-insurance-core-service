...
public class ReferenceDataService {
    ...
    public Mono<MasterReferenceDataResponse> getMasterReferenceData(String companyCode, String productCode) {
        ...
        return fetchAllCategoriesWithCaching(adapter, companyCode, productCode)
                .map(allCategories -> {
    ...
    public Mono<SingleCategoryReferenceDataResponse> getSingleCategoryReferenceData(String companyCode, String productCode, ReferenceCategory categoryKey) {
        log.info("Fetching single category reference data. Company: {}, Product: {}, Category: {}", 
                companyCode, productCode, categoryKey);
        long startTime = System.currentTimeMillis();

        InsuranceIntegrationAdapter adapter = getAdapter(companyCode);
        return getCategoryItemsWithCaching(adapter, companyCode, productCode, categoryKey)
                .map(items -> {
                    long duration = System.currentTimeMillis() - startTime;
    ...
    public Mono<DependentReferenceDataResponse> getDependentReferenceData(String companyCode, String productCode,
        ...
        InsuranceIntegrationAdapter adapter = getAdapter(companyCode);
        return getDependentItemsWithCaching(adapter, companyCode, productCode, parentCategoryKey, parentValue, childCategoryKey)
                .map(items -> {
                    long duration = System.currentTimeMillis() - startTime;
    ...
    private Mono<Map<ReferenceCategory, List<ReferenceDataItem>>> fetchAllCategoriesWithCaching(InsuranceIntegrationAdapter adapter, String companyCode, String productCode) {
        log.debug("Initiating fresh fetch for all categories from partner: {}", companyCode);
        
        return adapter.fetchMasterReferenceData(productCode)
                .doOnNext(freshData -> {
    ...
    private Mono<List<ReferenceDataItem>> getCategoryItemsWithCaching(InsuranceIntegrationAdapter adapter, String companyCode, String productCode, ReferenceCategory category) {
        String cacheKey = buildMasterCacheKey(companyCode, productCode, category);
        Cache cache = cacheManager.getCache(CachingConfig.MASTER_REFERENCE_DATA_CACHE);
        ...
        return fetchAllCategoriesWithCaching(adapter, companyCode, productCode)
                .map(allFreshData -> allFreshData.getOrDefault(category, Collections.emptyList()));
    }

    @SuppressWarnings("unchecked")
    private Mono<List<ReferenceDataItem>> getDependentItemsWithCaching(InsuranceIntegrationAdapter adapter, String companyCode, String productCode,
        ...
        return adapter.fetchDependentReferenceData(productCode, parentCategory, parentValue, childCategory)
                .doOnNext(freshItems -> {
